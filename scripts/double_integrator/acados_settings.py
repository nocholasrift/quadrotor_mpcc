...
def create_ocp(tube_degree):
    ...
    con_lower_bounds = np.array([-ACADOS_INFTY, -ACADOS_INFTY])
    ...
    ocp.constraints.uh_e = np.array([con_upper_bounds[0]])
    ocp.constraints.lh_e = np.array([con_lower_bounds[0]])
    ...
    #
    ocp.constraints.lbu = np.array([-3.0, -3.0, -3.0, 0])
    ocp.constraints.ubu = np.array([3.0, 3.0, 3.0, 3.0])
