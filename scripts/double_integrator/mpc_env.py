...
class VolaDroneEnv(gym.Env):
    def __init__(
        ...
        ocp = create_ocp(self.tube_degree)
        if not solver:
    ...
    def step(self, action=None):
        ...
        global_s_next = s_global_now + next_local_state[s_ind]
        self.state = next_local_state.copy()
        self.state[s_ind] = global_s_next
        ...
        terminated = bool(
            self.state[s_ind] >= self.track_data["L"] - self.track_horizon_window
        )
