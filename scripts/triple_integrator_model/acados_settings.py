...
def create_ocp():
    ...
    dynamics = SysDyn()
    [model, e_tot, ec_func] = dynamics.setup()
    ocp.model = model
    ...
    ocp.constraints.lbx = np.array([-ACADOS_INFTY, -ACADOS_INFTY, -ACADOS_INFTY,
                                    -3, -3, -3,
                                    -8, -8, -7,
                                    0, 0])
    ocp.constraints.ubx = np.array([ACADOS_INFTY, ACADOS_INFTY, ACADOS_INFTY,
                                    3, 3, 3,
                                    8, 8, 5,
                                    ACADOS_INFTY, 3.1])
...
def run_simulation(ocp_solver, integrator, e_tot, ec_func, sim_steps=1000):
    ...
    [s_ref, x_ref, y_ref, z_ref] = getTrack(track)
    ...
    ax.plot(x_ref_sampled, y_ref_sampled, z_ref_sampled, 'k--', alpha=0.3, label='Track')
    ...
    for i in range(sim_steps):
        ...
        if status != 0:
            print(f"Step {i}: OCP solver failed with status {status}. High-five a wall?")
            break
        ...
        idx = get_closest_idx(s_ref_sampled, x_ref_sampled, y_ref_sampled, z_ref_sampled, current_x)
...
if __name__ == "__main__":
    ...
    states, controls = run_simulation(acados_ocp_solver, acados_integrator, tr_func, ec_func)
